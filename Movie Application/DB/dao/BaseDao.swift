...
class BaseDao {
    ...
    func deleteExisitingItems<T> (withItemIds: [T],taskContext:NSManagedObjectContext,entityName:String,idName:String)  {
           let matchingRequest = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
           let organizationsIDs = withItemIds
           matchingRequest.predicate = NSPredicate(format: "\(idName) in %@",  organizationsIDs)
           let batchDeleteRequest = NSBatchDeleteRequest(fetchRequest: matchingRequest)
           batchDeleteRequest.resultType = .resultTypeObjectIDs
                  do {
                      let batchDeleteResult = try taskContext.execute(batchDeleteRequest) as? NSBatchDeleteResult
